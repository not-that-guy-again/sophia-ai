...
class MockDataStore:
    ...
    customers: dict[str, Customer] = {
        "CUST-001": Customer(
            "CUST-001", "jane.smith@example.com", "Jane Smith",
            "555-0101", NOW - timedelta(days=365), 3, 250.96, ["loyal"],
        ),
        "CUST-002": Customer(
            "CUST-002", "john.doe@example.com", "John Doe",
            "555-0102", NOW - timedelta(days=180), 1, 499.99,
        ),
        "CUST-003": Customer(
            "CUST-003", "alice.johnson@example.com", "Alice Johnson",
            "555-0103", NOW - timedelta(days=90), 2, 90.00,
        ),
        "CUST-004": Customer(
            "CUST-004", "bob.williams@example.com", "Bob Williams",
            None, NOW - timedelta(days=30), 1, 129.99,
        ),
        "CUST-005": Customer(
            "CUST-005", "carol.davis@example.com", "Carol Davis",
            "555-0105", NOW - timedelta(days=14), 1, 79.99,
        ),
    }

    orders: dict[str, Order] = {
        "ORD-12345": Order(
            "ORD-12345", "CUST-001", "delivered",
            [OrderItem("PROD-001", "Wireless Headphones", 1, 79.99, 79.99),
             OrderItem("PROD-002", "USB-C Cable", 2, 12.99, 25.98)],
            105.97, "USD", NOW - timedelta(days=14), NOW - timedelta(days=7),
            Address("123 Main St", "Springfield", "IL", "62701"),
            "1Z999AA10123456784",
        ),
        "ORD-67890": Order(
            "ORD-67890", "CUST-002", "processing",
            [OrderItem("PROD-003", "PlayStation 5", 1, 499.99, 499.99)],
            499.99, "USD", NOW - timedelta(days=1), NOW - timedelta(hours=6),
        ),
        "ORD-11111": Order(
            "ORD-11111", "CUST-003", "shipped",
            [OrderItem("PROD-004", "Laptop Stand", 1, 45.00, 45.00)],
            45.00, "USD", NOW - timedelta(days=3), NOW - timedelta(days=1),
            Address("456 Oak Ave", "Chicago", "IL", "60601"),
            "1Z999AA10123456785",
        ),
        "ORD-22222": Order(
            "ORD-22222", "CUST-003", "delivered",
            [OrderItem("PROD-004", "Laptop Stand", 1, 45.00, 45.00)],
            45.00, "USD", NOW - timedelta(days=45), NOW - timedelta(days=40),
            Address("456 Oak Ave", "Chicago", "IL", "60601"),
            "1Z999AA10123456786",
        ),
        "ORD-33333": Order(
            "ORD-33333", "CUST-001", "pending",
            [OrderItem("PROD-005", "Mechanical Keyboard", 1, 129.99, 129.99)],
            129.99, "USD", NOW - timedelta(hours=2), NOW - timedelta(hours=2),
        ),
        "ORD-44444": Order(
            "ORD-44444", "CUST-004", "confirmed",
            [OrderItem("PROD-005", "Mechanical Keyboard", 1, 129.99, 129.99)],
            129.99, "USD", NOW - timedelta(hours=12), NOW - timedelta(hours=10),
            Address("789 Pine Rd", "Austin", "TX", "73301"),
        ),
        "ORD-55555": Order(
            "ORD-55555", "CUST-005", "cancelled",
            [OrderItem("PROD-001", "Wireless Headphones", 1, 79.99, 79.99)],
            79.99, "USD", NOW - timedelta(days=10), NOW - timedelta(days=9),
        ),
        "ORD-66666": Order(
            "ORD-66666", "CUST-001", "delivered",
            [OrderItem("PROD-002", "USB-C Cable", 1, 12.99, 12.99)],
            12.99, "USD", NOW - timedelta(days=60), NOW - timedelta(days=55),
        ),
        "ORD-77777": Order(
            "ORD-77777", "CUST-005", "shipped",
            [OrderItem("PROD-001", "Wireless Headphones", 1, 79.99, 79.99)],
            79.99, "USD", NOW - timedelta(days=5), NOW - timedelta(days=3),
            Address("321 Elm St", "Denver", "CO", "80201"),
            "1Z999AA10123456787",
        ),
        "ORD-88888": Order(
            "ORD-88888", "CUST-002", "delivered",
            [OrderItem("PROD-002", "USB-C Cable", 3, 12.99, 38.97)],
            38.97, "USD", NOW - timedelta(days=20), NOW - timedelta(days=15),
        ),
    }
    ...
    shipments: dict[str, ShipmentTracking] = {
        "ORD-12345": ShipmentTracking(
            "ORD-12345", "UPS", "1Z999AA10123456784", "delivered",
            NOW - timedelta(days=7), "Springfield, IL",
        ...
        "ORD-11111": ShipmentTracking(
            "ORD-11111", "FedEx", "1Z999AA10123456785", "in_transit",
            NOW + timedelta(days=2), "St. Louis, MO",
        ...
        "ORD-77777": ShipmentTracking(
            "ORD-77777", "USPS", "1Z999AA10123456787", "in_transit",
            NOW + timedelta(days=1), "Kansas City, MO",
            [TrackingEvent(NOW - timedelta(days=3), "Houston, TX", "Picked up", "picked_up"),
             TrackingEvent(NOW - timedelta(days=1), "Kansas City, MO", "In transit", "in_transit")],
        ),
    }

    returns: dict[str, ReturnStatus] = {
        "RET-001": ReturnStatus(
            "RET-001", "ORD-22222", "processed",
            [ReturnItem("PROD-004", 1, "changed_mind")],
            NOW - timedelta(days=30), 45.00,
        ),
        "RET-002": ReturnStatus(
            "RET-002", "ORD-66666", "in_transit",
...
class MockOrderService(OrderService):
    ...
    async def get_order_status(self, order_id: str) -> OrderStatus | None:
        ...
        return OrderStatus(
            ...
            estimated_delivery=(
                ...
                if order_id in MockDataStore.shipments else None
            ),
    ...
    async def place_order(self, customer_id: str, items: list[OrderItem]) -> Order:
        ...
        order = Order(
            order_id=order_id, customer_id=customer_id, status="pending",
            items=items, total=total, currency="USD",
...
class MockCustomerService(CustomerService):
    ...
    async def search_customers(self, query: str) -> list[Customer]:
        q = query.lower()
        return [
            c for c in MockDataStore.customers.values()
    ...
    async def get_customer_history(self, customer_id: str) -> CustomerHistory:
        ...
        returns = [
            r for r in MockDataStore.returns.values()
            if any(o.customer_id == customer_id for o in MockDataStore.orders.values()
                   if o.order_id == r.order_id)
        ]
        total_refunded = sum(r.refund_amount or 0.0 for r in returns)
        return CustomerHistory(
            customer=customer, orders=orders, returns=returns,
...
class MockInventoryService(InventoryService):
    ...
    async def check_warranty_status(self, order_id: str, product_id: str) -> WarrantyStatus:
        ...
        return WarrantyStatus(
            ...
            is_active=is_active, coverage_type="standard",
        )
...
class MockCompensationService(CompensationService):
    ...
    async def process_partial_refund(
        ...
        return RefundResult(
            ...
            status="processed", method="original_payment",
        )
    ...
    async def initiate_return(
        ...
        label_url = (
            f"https://labels.example.com/{uuid.uuid4().hex[:8]}" if has_defective else None
        )
        ret = ReturnStatus(
            return_id=return_id, order_id=order_id, status="initiated",
            items=items, created_at=datetime.now(),
        )
        MockDataStore.returns[return_id] = ret
        instructions = (
            "Ship items back using the provided label." if has_defective
        ...
        return ReturnInitiationResult(
            ...
            return_label_url=label_url, status="initiated",
